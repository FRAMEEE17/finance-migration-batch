"""Quality gate in front of fact_gl_line. Ticket 5.

Runs 5 blocking + 3 non-blocking checks over one scope, logs every
finding to dq_violations. Blocking findings are excluded from
load_fact.py's insert; non-blocking ones are logged only.

Run standalone: python src/quality_gate.py
(also imported by load_fact.py, which calls run_gate() inside its own
transaction before inserting into fact_gl_line)
"""
...
def main() -> int:
    if len(sys.argv) != 4:
        print("usage: python src/quality_gate.py <company_code> <fiscal_year> <fiscal_period>")
        return 1
    ...
    counts = run_gate(con, scope_filter)
    con.close()
